/* airlinefyi-embed v1.0.0 | MIT | https://widget.airlinefyi.com */
...
function viewLink(url, label, _config) {
  return `<div class="transportfyi-view-link"><a href="${esc(url)}" target="_blank" rel="noopener">${esc(label)} ${externalLinkIcon}</a></div>`;
}
...
function renderGlossaryCard(data, config) {
  ...
  const name = String((_c = (_b = (_a = data.term) != null ? _a : data.name) != null ? _b : data.slug) != null ? _c : "");
  const definition = String((_f = (_e = (_d = data.definition) != null ? _d : data.short_description) != null ? _e : data.short_definition) != null ? _f : "");
  const abbr = data.abbreviation ? String(data.abbreviation) : "";
  const categoryLabel = String((_h = (_g = data.category_name) != null ? _g : data.category) != null ? _h : "");
  const slug = String((_i = data.slug) != null ? _i : "");
  let termUrl;
  if (config.site === "airportfyi") {
    termUrl = `https://${config.domain}/glossary/${esc(slug)}/`;
  } else {
    termUrl = `https://${config.domain}/glossary/${esc(slug)}/`;
  }
  ...
  return `
    <div class="transportfyi-header">
      <div>
        <div class="transportfyi-header-title">${esc(name)}${abbr ? ` <span class="transportfyi-badge" style="background:${config.accent};color:#fff">${esc(abbr)}</span>` : ""}</div>
        ${categoryLabel ? `<div class="transportfyi-header-subtitle">${esc(categoryLabel)}</div>` : ""}
      </div>
    </div>
    <div class="transportfyi-body" style="font-size:0.9rem;line-height:1.5;">
      ${esc(definition)}
    </div>
    <div class="transportfyi-view-link"><a href="${termUrl}" target="_blank" rel="noopener">${esc(name)} ${externalLinkIcon}</a></div>
    <div class="transportfyi-view-link"><a href="${glossaryUrl}" target="_blank" rel="noopener">Full glossary on ${esc(config.name)} ${externalLinkIcon}</a></div>
...
function renderGuideCard(data, config) {
  ...
  const summary = String((_e = (_d = data.summary) != null ? _d : data.description) != null ? _e : "");
  const slug = String((_f = data.slug) != null ? _f : "");
  const seriesName = data.series_name ? String(data.series_name) : "";
  const readingTime = data.reading_time_minutes ? Number(data.reading_time_minutes) : null;
  const guideUrl = `https://${config.domain}/guides/${esc(slug)}/`;
  return `
    <div class="transportfyi-header">
      <div>
        <div class="transportfyi-header-title">${esc(title)}</div>
        <div class="transportfyi-header-subtitle">${seriesName ? esc(seriesName) : "Guide"}${readingTime ? ` \xB7 ${readingTime} min read` : ""}</div>
      </div>
    </div>
    ${summary ? `<div class="transportfyi-desc">${esc(summary)}</div>` : ""}
...
function injectDefinedTerm(data, domain, siteName) {
  if (document.querySelector('script[data-transportfyi-snippet="term"]')) return;
  const jsonLd = {
  ...
  script.setAttribute("data-transportfyi-snippet", "term");
...
function getGlossaryPath(config, slug) {
  switch (config.site) {
    case "airportfyi":
...
function initGlossaryWidget(el, config) {
  ...
  const slug = (_a = dataset.slug) != null ? _a : "";
  const shadow = createShadow(el, config);
  const container = createWidgetRoot(shadow, el, "transportfyi-glossary-widget");
  if (!slug) {
    renderError(container, "Missing data-slug attribute.", config);
    return;
  }
  const path = getGlossaryPath(config, slug);
  if (!path) {
    renderError(container, `Glossary not available for ${config.name}.`, config);
    return;
  }
  renderLoading(container);
  fetchAPI(config.apiBase, path).then((data) => {
    ...
    if (dataset.noSnippet !== "true") {
      const term = String((_b = (_a2 = data.term) != null ? _a2 : data.name) != null ? _b : "");
      const def = String((_e = (_d = (_c = data.definition) != null ? _c : data.short_description) != null ? _d : data.short_definition) != null ? _e : "");
      if (term && def) {
        injectDefinedTerm({ name: term, definition: def }, config.domain, config.name);
      }
    }
  }).catch(() => {
...
function initGuideWidget(el, config) {
  ...
  const slug = (_a = dataset.slug) != null ? _a : "";
  const shadow = createShadow(el, config);
  const container = createWidgetRoot(shadow, el, "transportfyi-guide-widget");
  if (!slug) {
    renderError(container, "Missing data-slug attribute.", config);
    return;
  }
  if (config.site !== "airlinefyi") {
    renderError(container, `Guides not available for ${config.name}.`, config);
    return;
  }
  renderLoading(container);
...
// src/inline/route-badge.ts
function initRouteBadgeWidget(el, config) {
  var _a;
  ...
  fetchAPI(config.apiBase, `airlines/${slug}/`).then((data) => {
    var _a2, _b;
    const routes = Number((_a2 = data.route_count) != null ? _a2 : 0);
    const name = String((_b = data.name) != null ? _b : slug);
    container.innerHTML = `<span class="transportfyi-badge" style="background:${config.accent};color:#fff">${esc(name)}: ${fmtNum(routes)} routes</span>`;
  }).catch(() => {
    container.innerHTML = "";
  });
}

// src/_entry_airlinefyi.ts
function initWidget(el, type, config) {
  ...
  switch (type) {
    ...
    case "faq":
      ...
      break;
    case "route-badge":
      initRouteBadgeWidget(el, config);
      break;
    default:
      break;
  }
...
(function bootstrap() {
  const config = '{"site":"airlinefyi","name":"AirlineFYI","domain":"airlinefyi.com","accent":"#3B82F6","attribute":"data-airlinefyi","apiBase":"https://airlinefyi.com/api/v1/","searchPath":"/search/","entityName":"Airlines","entitySlug":"airlines"}';
...
function makeWidgetElement(widgetType, initFn, domainAttrs) {
  ...
  return class extends HTMLElement {
    ...
    connectedCallback() {
      ...
      this._syncDataAttrs();
      initFn(this, '{"site":"airlinefyi","name":"AirlineFYI","domain":"airlinefyi.com","accent":"#3B82F6","attribute":"data-airlinefyi","apiBase":"https://airlinefyi.com/api/v1/","searchPath":"/search/","entityName":"Airlines","entitySlug":"airlines"}');
    }
    ...
    _syncDataAttrs() {
      const attrKey = '{"site":"airlinefyi","name":"AirlineFYI","domain":"airlinefyi.com","accent":"#3B82F6","attribute":"data-airlinefyi","apiBase":"https://airlinefyi.com/api/v1/","searchPath":"/search/","entityName":"Airlines","entitySlug":"airlines"}'.attribute.replace("data-", "");
...
(function registerElements() {
  ...
  const site = '{"site":"airlinefyi","name":"AirlineFYI","domain":"airlinefyi.com","accent":"#3B82F6","attribute":"data-airlinefyi","apiBase":"https://airlinefyi.com/api/v1/","searchPath":"/search/","entityName":"Airlines","entitySlug":"airlines"}'.site;
  const defs = [
